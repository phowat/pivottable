...
(function() {
  var $, PivotData, addSeparators, aggregatorTemplates, aggregators, dayNamesEn, derivers, locales, mthNamesEn, naturalSort, numberFormat, pivotTableRenderer, pivotTableRendererMany, renderers, usFmt, usFmtInt, usFmtPct, zeroPad,
  ...
  aggregatorTemplates = {
    ...
    countUniqueNotEmpty: function(formatter) {
      if (formatter == null) {
        formatter = usFmtInt;
      }
      return function(arg) {
        ...
        return function(data, rowKey, colKey) {
          return {
            ...
            push: function(record) {
              var ref;
              if ((record[attr] != null) && record[attr].length > 0 && (ref = record[attr], indexOf.call(this.uniq, ref) < 0)) {
                return this.uniq.push(record[attr]);
              }
            },
            value: function() {
    ...
    countUniqueOverSum: function(formatter) {
      if (formatter == null) {
        formatter = usFmt;
      }
      return function(arg) {
        var attr, denom;
        attr = arg[0], denom = arg[1];
        return function(data, rowKey, colKey) {
          return {
            uniqNum: [],
            sumDenom: 0,
            push: function(record) {
              var ref;
              if (ref = record[attr], indexOf.call(this.uniqNum, ref) < 0) {
                this.uniqNum.push(record[attr]);
              }
              if (!isNaN(parseFloat(record[denom]))) {
                return this.sumDenom += parseFloat(record[denom]);
              }
            ...
            value: function() {
              return this.uniqNum.length / this.sumDenom;
            },
            format: formatter,
            numInputs: (typeof num !== "undefined" && num !== null) && (denom != null) ? 0 : 2
          };
        };
      };
    },
  ...
  renderers = {
    ...
    "Table (Multiple Aggregators)": function(pvtData, opts) {
      return pivotTableRendererMany(pvtData, opts);
    },
  ...
  PivotData = (function() {
    function PivotData(input, opts) {
      ...
      var aggregator, l, len1, ref;
      this.aggregator = opts.aggregator;
      this.aggregatorName = opts.aggregatorName;
      this.aggregatorsList = [];
      ...
      this.allTotals = [];
      this.sorted = false;
      if (opts.aggregatorsList != null) {
        ref = opts.aggregatorsList;
        for (l = 0, len1 = ref.length; l < len1; l++) {
          aggregator = ref[l];
          this.aggregatorsList.push(aggregators[aggregator.name](aggregator.vals));
          this.allTotals.push(aggregators[aggregator.name](aggregator.vals)(this, [], []));
          this.aggregatorNames.push(aggregator.description);
        }
      }
    ...
    PivotData.prototype.processRecord = function(record) {
      var a, agg, colKey, flatColKey, flatRowKey, l, len1, len2, len3, len4, len5, n, o, q, ref, ref1, ref2, ref3, ref4, ref5, ref6, rowKey, s, t, x;
      colKey = [];
      rowKey = [];
      ref = this.colAttrs;
      for (l = 0, len1 = ref.length; l < len1; l++) {
      ...
      if (rowKey.length !== 0) {
        if (this.aggregatorsList.length > 0) {
          if (!this.rowTotalsList[flatRowKey]) {
            this.rowTotalsList[flatRowKey] = [];
            ref5 = this.aggregatorsList;
            for (q = 0, len4 = ref5.length; q < len4; q++) {
              agg = ref5[q];
              this.rowTotalsList[flatRowKey].push(agg(this, rowKey, []));
            }
          }
          ref6 = this.rowTotalsList[flatRowKey];
    ...
    PivotData.prototype.getAggregator = function(rowKey, colKey, multi) {
      ...
      if (rowKey.length === 0 && colKey.length === 0) {
        ...
        if (multi === true) {
          agg = this.rowTotalsList[flatRowKey];
        } else {
  ...
  pivotTableRendererMany = function(pivotData, opts) {
    var aggregator, agname, defaults, i, j, l, len1, len2, len3, n, o, r, ref, result, rowAttrs, rowKey, rowKeys, spanSize, td, th, totalAggregator, totalAggregators, tr, txt, val, x;
    defaults = {
      localeStrings: {
        totals: "Totals"
      }
    ...
    for (i in rowKeys) {
      ...
      if (pivotData.aggregatorsList.length > 0) {
        ...
        val = null;
        for (n = 0, len2 = totalAggregators.length; n < len2; n++) {
          aggregator = totalAggregators[n];
          val = aggregator.value();
          td = document.createElement("td");
          td.className = "pvtTotal rowTotal";
          td.innerHTML = aggregator.format(val);
    ...
    if (pivotData.aggregatorsList.length > 0) {
      ...
      val = null;
      for (o = 0, len3 = totalAggregators.length; o < len3; o++) {
        aggregator = totalAggregators[o];
        val = aggregator.value();
        td = document.createElement("td");
        td.className = "pvtGrandTotal";
        td.innerHTML = aggregator.format(val);
  ...
  $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
    var a, aggregator, attrLength, axisValues, c, colList, defaults, e, existingOpts, fn, i, initialRender, k, l, len1, len2, len3, len4, n, o, opts, pivotTable, q, ref, ref1, ref2, ref3, ref4, refresh, refreshDelayed, renderer, rendererControl, shownAttributes, tblCols, tr1, tr2, uiTable, unusedAttrsVerticalAutoOverride, x;
    if (overwrite == null) {
      overwrite = false;
    }
    if (locale == null) {
    ...
    try {
      ...
      refreshDelayed = (function(_this) {
        return function() {
          var attr, exclusions, len5, natSort, newDropdown, numInputsToProcess, pivotUIOptions, pvtVals, ref5, ref6, s, subopts, u, unusedAttrsContainer, vals;
          subopts = {
